				...
				if grid[(x,y)].cart and grid[(x,y)].cart.last_update < tick:
					...
					if grid[(x,y)].intersection():
						...
						if grid[(x,y)].cart.next_turn == 0:
							grid[(x,y)].cart.facing = LEFT_DIRECTION[grid[(x,y)].cart.facing]
						elif grid[(x,y)].cart.next_turn == 2:
							grid[(x,y)].cart.facing = RIGHT_DIRECTION[grid[(x,y)].cart.facing]
						...
						way_out = [x for x in grid[(x,y)].exits if x != OPPOSITES[facing]][0]
