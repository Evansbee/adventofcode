        ...
        for y in range(min(y_vals), max(y_vals) + 1):
            ...
            for x in range(min(x_vals), max(x_vals) + 1):
                if (x, y) in self.units:
                    ...
                    row_info += [
                        f"{self.units[(x,y)]}({self.units[(x,y)].hp}[{self.units[(x,y)].ap}])"
                    ]
                else:
        ...
        if self.running:
            ...
            for x, y in self.each_position():
                if (x, y) in self.units and self.units[(x, y)].last_update < self.tick:
                    ...
                    enemy_positions = [
                        k for k, v in self.units.items() if v.type == enemy_type
                    ]
                    ...
                    enemy_positions_in_range = set(enemy_positions) & set(
                        neighbor4((x, y))
                    )
                    if len(enemy_positions_in_range) == 0:
                        ...
                        if len(paths) > 0:
                            ...
                            potential_move_to_targets = list(
                                set([p[-1] for p in paths])
                            )
                            potential_move_to_targets.sort(
                                key=cmp_to_key(sort_position)
                            )
                            ...
                            for potential_first_move in neighbor4(actual_unit_position):
                                if self.position_clear(potential_first_move):
                                    path = a_star(
                                        potential_first_move, move_to_target, self
                                    )
                    ...
                    enemy_positions_in_range = set(enemy_positions) & set(
                        neighbor4(actual_unit_position)
                    )
                    if len(enemy_positions_in_range) > 0:
