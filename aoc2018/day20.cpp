...
std::string test1 = "^ENNWSWW(NEWS|)SSSEEN(WNSE|)EE(SWEN|)NNN$";
class tree_node;
std::vector<tree_node *> tree_stack;
...
class tree_node {
    ...
    tree_node(tree_node *parent):m_parent(parent),id(next_id++){}
    tree_node() :tree_node(NULL){
    }

    tree_node *add_element(char val)
    {
        std::cout<<id<<": ";
        if(val=='N' || val == 'W' || val == 'E' || val == 'S')
        {
            //append;
            if(m_children.size() == 0)
            {
                std::cout<<"Direct Append: "<<val<<std::endl;
                m_directions.push_back(val);
            }
            else
            {std::cout<<"Child Append: "<<val<<std::endl;
        ...
        {
            std::cout<<"Making Neighbor"<<std::endl;
            //change current to a neighbor
            auto new_tree = new tree_node(this);
            m_parent->m_children.push_back(new_tree);
            return new_tree;
        }
        else if(val == '(')
        {
            if(m_children.size() == 0)
            {
                ...
                auto new_tree = new tree_node(this);
                m_children.push_back(new_tree);
                std::cout<<"Making Child ("<<new_tree->id<<")"<<std::endl;
                return new_tree;
            }
            else
            {
        ...
        else if(val == ')')
        {
            ...
            auto element = tree_stack[tree_stack.size()-1];
            tree_stack.pop_back();
            return element;
        }
        else
    ...
    size_t shortest_path(){
        size_t test = m_directions.size();


        if(m_children.size() > 0)
        {std::vector<size_t> children_sizes;
        for(auto& x: m_children)
        {
            children_sizes.push_back(x->shortest_path());
        }
            test += (*std::min_element(children_sizes.begin(), children_sizes.end()));
        }
        return test;
    }
    size_t longest_path(){
        ...
        for(auto& x: m_children)
        {
            ...
            test += (*std::max_element(children_sizes.begin(), children_sizes.end()));
        }
        return test;
    }
};
size_t tree_node::next_id = 0;
...
{
    std::size_t ptr = 1;
    while(test1[ptr] != '$' && test1[ptr] != '\0')
    {
        current = current->add_element(test1[ptr]);
        ptr++;
    }
    std::cout<<head->shortest_path()<<std::endl;
    std::cout<<head->longest_path()<<std::endl;
    return 0;
}