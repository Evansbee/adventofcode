...
def printgrid(grid, row=None, span=10):
...
def problem1(problem_input):
    ...
    while len(cursor) > 0:
        ...
        if current_spout in grid and grid[current_spout] == "F":
            continue

        if len(cursor) >= 1900:
            print(f"Cursors: {len(cursor)} Spout: {current_spout}")
            printgrid(grid, current_spout[1])
        ...
        if todown[1] > max_y:
            ...
            if below(l) not in directions_water_can_flow_from_here(grid, l) and below(
                r
            ) not in directions_water_can_flow_from_here(grid, r):
                fill_h_line(grid, l[0], r[0], current_spout[1], "~")
            else:
                if grid[r] == "F" or grid[l] == "F":
                    fill_h_line(grid, l[0], r[0], current_spout[1], "F")
                else:
                    fill_h_line(grid, l[0], r[0], current_spout[1], "|")
    ...
    printgrid(grid)
    waters = [
        v for k, v in grid.items() if k[1] <= max_y and k[1] >= min_y and v in "|.F~"
    ]
    still = [v for k, v in grid.items() if k[1] <= max_y and k[1] >= min_y and v in "~"]

    return len(waters), len(still)
