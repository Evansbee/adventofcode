...
class PlanetInformation:
	...
	def state(self):
		return self.p[0] * 2 + self.p[1] *2 + self.p[2] * 3 + self.v[0] * 5 + self.v[1] * 7 + self.v[2] * 11 + self.id * 13
...
def find_cycle(planets, idx):
	...
	while True:
		if (ticks % 100000) == 0:
			...
			if ticks != 0:
				per = total / ticks
				print(f'{ticks} iterations complete ({total:0.2f}s - {1.0/per:0.0f} iterations/s)...')
		ticks+=1
		for a,b in combos:
...
def problem2(problem_input):
	...
	for x in range(min(x_cycle,y_cycle,z_cycle), x_cycle * y_cycle * z_cycle + 1,min(x_cycle,y_cycle,z_cycle)):
